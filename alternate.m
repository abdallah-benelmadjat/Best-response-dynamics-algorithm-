...
%         end        
        ...
        if ((((((congestion(1,1)+1)/10) + 15 + 10 + ((congestion(1,1)+1)/20))) < playerscost(1:m,1)) | ((15+((congestion(3,1)+1)/5)) < playerscost(1:m,1)) | ((((congestion(4,1)+1)/10) + 15 + 11 + 12) < playerscost(1:m,1)) | ((15+((congestion(7,1)+1)/8)+12) < playerscost(1:m,1))) == 0
            'no better option';
        end
        if ((((congestion(1,1)+1)/10) + 15 + 10 + ((congestion(1,1)+1)/20))) < playerscost(1:m,1) & memory(1:m,1) ~= 1
            memory(1:m,1:3)=0;
            memory(1:m,1)=1;
            memory(1:m,2)=2;
            'found better option in path 1';
        end
        if (15+((congestion(3,1)+1)/5)) < playerscost(1:m,1) & memory(1:m,1) ~= 3
            memory(1:m,1:3)=0;
            memory(1:m,1)=3;
            'found better option in path 2';
        end
        if (((congestion(4,1)+1)/10) + 15 + 11 + 12) < playerscost(1:m,1) & memory(1:m,1) ~= 4
            memory(1:m,1:3)=0;
            memory(1:m,1)=4;
            memory(1:m,2)=5;
            memory(1:m,3)=6;
        ...
        if (15+((congestion(7,1)+1)/8)+12) < playerscost(1:m,1) & memory(1:m,1) ~= 7
            memory(1:m,1:3)=0;
            memory(1:m,1)=7;
            memory(1:m,2)=8;
